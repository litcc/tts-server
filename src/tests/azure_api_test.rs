...
use crate::{
    utils::{
        azure_api::{
            AzureApiEdgeFree, AzureApiNewWebsocket,
            AzureApiRegionIdentifier, AzureApiSubscribeToken, AzureAuthKey, VoicesItem,
        },
        log::init_test_log,
    },
    AppArgs,
...
async fn test_azure_api_subscribe_create_oauth_token() {
    ...
    let kk = AzureApiSubscribeToken::new(
        AzureApiRegionIdentifier::from(&region).unwrap(),
        &subscription_key,
    );
...
async fn test_azure_api_serde_voices_list_data() -> anyhow::Result<()> {
    ...
    let voice_list: Vec<VoicesItem> = serde_json::from_slice(body.as_ref()).map_err(|e| {
        error!("{:?}", e);
        e
    })?;
    ...
    let voice_list2: Vec<VoicesItem> = serde_json::from_slice(body2.as_ref()).map_err(|e| {
        error!("{:?}", e);
        e
    })?;
