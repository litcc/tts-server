...
use crate::{
    web::{entity::ApiBaseResponse, utils::get_request_body_to_entity},
    AppArgs,
};
    ...
    fn call(&self, mut req: ServiceRequest) -> Self::Future {
        ...
        let req_path_token = {
            ...
            query_list.into_iter().for_each(|i| {
                ...
                let key_name = item.first().unwrap();
                if key_name == &"token" {
        ...
        if req.request().method().as_str() == "POST" {
            Box::pin(async move {
                ...
                return if let Ok(data) = body_entity {
                    ...
                    let body = EitherBody::new(
                        ApiBaseResponse::<()>::error_by_status_code(500, "解析请求中的 Token 错误")
                            .to_string(),
                    );
            ...
            Box::pin(async move {
                let body = EitherBody::new(
                    ApiBaseResponse::<()>::error_by_status_code(403, "Token 错误，无法进行调用")
                        .to_string(),
                );
